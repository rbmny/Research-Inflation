...
def lasso_matrix(e):
    with open(f'{path}/{e}', 'rb') as handle:
        ...
        # matrix2['predict'] = temp[:-1]
        df_predict = pd.DataFrame(columns=['date','alpha', 'l1_ratio', 'predict', 'actual', 'mse', 'coef', 'intercept', 'c1'])
    ...
    with parallel_backend('threading', n_jobs=8):
        ...
        for i in range(47, len(temp)-1):
            ...
            best_model = ElasticNet(alpha=.00002, l1_ratio=0.5, max_iter=1000)
            best_model.fit(X, y)
            ...
            row = [matrix2.index[i+1], best_model.alpha, best_model.l1_ratio, yhat, actual, mse, best_model.coef_, best_model.intercept_, data[i+1, 0]]
            df_predict.loc[i] = row

    with open(f'{path}/Elastic/e_predict_{e}', 'wb') as handle:
