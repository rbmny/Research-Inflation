...
def lasso_matrix(e):
    ...
    with parallel_backend('threading', n_jobs=8):
        ...
        with open(f'../Databases/Data/Matrices/Linear/l_predict_2_{e}', 'rb') as f:
            coef_file = pickle.load(f)
        ...
        for coef_index in coef_dates:
            ...
            for i in range(47, len(temp)-1):
                    ...
                    row = [matrix2.index[i+1], yhat, actual, mse, best_model.coef_]
                    df_predict.loc[i] = row
            try:
                os.mkdir(f'{path}/Linear/Coefs/{e[:-7]}')
            except FileExistsError:
                print(f'Linear/Coefs/full_{e[:-7]}/{e}')
            with open(f'{path}/Linear/Coefs/{e[:-7]}/e_{coef_index}_predict_{e}', 'wb') as handle:
...
if __name__ == '__main__':
    lasso_matrix('constraint_coefs_matrix_15k.pickle')
