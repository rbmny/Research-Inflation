...
def lasso_matrix(e):
    with open(f'{path}/{e}', 'rb') as handle:
        ...
        # matrix2['predict'] = temp[:-1]
        df_predict = pd.DataFrame(columns=['date','alpha', 'l1_ratio', 'predict', 'actual', 'mse', 'coef', 'intercept'])
    ...
    with parallel_backend('threading', n_jobs=8):
        ...
        for i in range(47, len(temp)-1):
            ...
            X, y = data[i-47:i, :-1], data[i-47:i, -1]
            scaler = StandardScaler()
            scaler.fit(X)
            ...
            best_model = ElasticNet(alpha=.000001)
            multi = MultiOutputRegressor(best_model)
            multi.fit(X, y)

            row = scaler.transform((data[i + 1, :-1]))
            yhat = multi.predict(row)
            actual = data[i + 1:i+12, -1]
            mse = mean_squared_error(actual, yhat)

            row = [matrix2.index[i+1], best_model.alpha, best_model.l1_ratio, yhat, actual, mse, best_model.coef_, best_model.intercept_]
            df_predict.loc[i] = row

    with open(f'{path}/Elastic/{best_model.alpha}_rolling_adjusted_scaled_e_predict_{e}', 'wb') as handle:
